...
Partial Class frmOvertimeRequestAction
    Inherits MainPage
...
#Region "Protected Sub"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            End If
            If CanEdit Then
                txtHoursNo.Enabled = False
                txtMinutsNo.Enabled = False
                txtConfirmedHours.Enabled = True
                txtConfirmedMinuts.Enabled = True
                OverTimeDate.Enabled = True
                ddlOverTimeType.Enabled = True
            Else
                txtHoursNo.Enabled = False
                txtMinutsNo.Enabled = False
                txtConfirmedHours.Enabled = False
                txtConfirmedMinuts.Enabled = False
                OverTimeDate.Enabled = False
                ddlOverTimeType.Enabled = False
            End If
    ...
    Private Sub FillEmployeeVacations()
        Try
            ...
            Else
                ...
                requesttypestr = "Select " & SelectRequestType & " from SS_RequestTypes where RequestCode='SS_001919'"
            ...
            Dim sourceTable As DataTable = DS1.Tables(0)
            Dim filteredTable As DataTable = sourceTable.Clone()

            Dim rows() As DataRow = sourceTable.Select("Action NOT LIKE 'Pending%'")
            ...
            If rows.Length > 0 Then
                txtConfirmedHours.Text = filteredTable.Rows(filteredTable.Rows.Count - 1)("ConfirmedNoOfdays").ToString()
                txtConfirmedMinuts.Text = filteredTable.Rows(filteredTable.Rows.Count - 1)("MinutsCount").ToString()
            Else
                txtConfirmedHours.Text = txtHoursNo.Text
                txtConfirmedMinuts.Text = txtMinutsNo.Text
            End If
...
#Region "Private Function"
    ...
    Private Function SaveOverTimePayabilities(ByVal employeeId As Integer) As Boolean
        ...
        ClsTransactionsType.Find(" Code=" & ddlOverTimeType.SelectedItem.Value)

        ClsEmployeesPayability.TransactionTypeID = ClsTransactionsType.ID
        ClsEmployeesPayability.Number = StrSerial
        ClsEmployeesPayability.TransactionDate = ClsEmployeesPayability.SetHigriDate(OverTimeDate.Value)
        ClsEmployeesPayability.TransactionComment = TxtRemarks.Text
        ClsEmployeesPayability.SalaryLink = "True"

        Dim RequestSerial As Integer = Request.QueryString.Item("RequestSerial")
        Dim notes As String = "Request Id " & RequestSerial & " .. Due date=" & ClsEmployeesPayability.SetHigriDate(OverTimeDate.Value) & " .. Hours=" & txtConfirmedHours.Text & " .. Minuts=" & txtConfirmedMinuts.Text
        ClsEmployeesPayability.TransactionComment = notes
        ClsEmployeesPayability.Src = "SS_001919"
    ...
    Private Function SaveInstalment(ByVal IntId As Integer, ByVal empId As Integer) As Boolean
        ...
        Try
            ...
            fiscalperiood.Find("FromDate <= '" & ClsEmployees.SetHigriDate(OverTimeDate.Value) & "' and ToDate >='" & ClsEmployees.SetHigriDate(OverTimeDate.Value) & "'")
            ...
            Calculate_SalaryPerHour(Amount, empId, ClsEmployees.SetHigriDate(OverTimeDate.Value))
